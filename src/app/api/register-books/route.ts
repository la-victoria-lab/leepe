...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: existingBooks } = await auth.supabase.from('libros').select('isbn, titulo').in('isbn', validIsbns)
    ...
    return NextResponse.json({ error: `Error al procesar la solicitud: ${errorMessage}` }, { status: 500 })
  }
}
