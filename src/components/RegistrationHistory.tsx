    ...
    <div className='border rounded-xl p-4 bg-white shadow'>
      ...
      {!items.length && !isLoading ? (
        ...
        <ul className='space-y-2'>
          {items.map((book) => (
            <li key={`${book.isbn}-${book.fecha_registro || ''}`} className='text-sm flex items-baseline justify-between gap-4'>
              <span className='text-gray-900'>{book.titulo}</span>
              <span className='text-gray-500'>
                {book.fecha_registro ? new Date(book.fecha_registro).toLocaleString('es') : ''}
              </span>
            </li>
          ))}
        </ul>
